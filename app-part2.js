...
// Ranks weather codes by how "significant" they are, so when we need one representative
// code for a whole period (e.g. all the daytime hours of a given day), the most notable
// condition wins rather than just whichever hour happens to be picked.
function weatherCodeSeverityRank(code) {
    if (code >= 95 && code <= 99) return 100; // thunderstorm
    if ((code >= 71 && code <= 77) || code === 85 || code === 86) return 90; // snow
    if ([65, 67, 82].includes(code)) return 85; // heavy rain
    if ([63, 66, 81, 53].includes(code)) return 75; // moderate rain
    if ([51, 55, 56, 57, 61, 80].includes(code)) return 65; // light rain
...
// Independently determines the day-period and night-period weather condition for a given
// calendar day, using only that period's own hourly data — the day's code is never reused
// for the night icon (and vice versa).
function computeDayNightPeriodConditions(dailyData, hourlyData, dayIndex) {
    const targetDate = dailyData.time[dayIndex];
    const fallbackIsNightForDate = false; // used only if a hour's date can't be matched to a day
    ...
    // Fallback to the single daily code only if a period had no matching hourly data at all
    // (shouldn't normally happen, but keeps the icon from ever going blank).
